...
//  ndays_txpp : calcule le nombre de journée supérieur à 30°C
...
const char *ndays_txpp (const char *filename, long int *msrptr);

#define MSR_FORMAT  "%ld day(s)"
...
const char *ndays_txpp(const char *filename, long int *msrptr) {
  ...
  //    && k == (nombre de valeur lu sur f ou tx est supérieur ou égal à 30)
